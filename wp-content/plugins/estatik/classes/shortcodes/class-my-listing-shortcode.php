...
class Es_My_Listing_Shortcode extends Es_Shortcode
{
    ...
    public function build_query_args( $atts )
    {
	    ...
	    if ( ! empty( $atts['state'] ) ) {
        	$atts['state'] = explode( ',', $atts['state'] );
		    $query_args['meta_query'][] = array( 'key' => '_address_component_administrative_area_level_1', 'value' => $atts['state'], 'compare' => 'IN' );
	    }

	    if ( ! empty( $atts['country'] ) ) {
		    $atts['country'] = explode( ',', $atts['country'] );
		    $query_args['meta_query'][] = array( 'key' => '_address_component_country', 'value' => $atts['country'], 'compare' => 'IN' );
	    }

	    if ( ! empty( $atts['city'] ) ) {
		    $atts['city'] = explode( ',', $atts['city'] );
		    $query_args['meta_query'][] = array( 'key' => '_address_component_locality', 'value' => $atts['city'], 'compare' => 'IN' );
	    }

	    if ( ! empty( $atts['neighborhood'] ) ) {
		    $atts['neighborhood'] = explode( ',', $atts['neighborhood'] );
		    $query_args['meta_query'][] = array( 'key' => '_address_component_neighborhood', 'value' => $atts['neighborhood'], 'compare' => 'IN' );
	    }
